...
//  SwiftUIFundamentalProjectTests
//
//  Created by Panachai Sulsaksakul on 10/28/24.
//

import Foundation
import XCTest
@testable import SwiftUIFundamentalProject

class JSONMapperTests: XCTestCase {
    func test_with_valid_json_successfully_decodes() {
        // Check whether a static json file is correct
        ...
        // Check whether the response is not nil
        ...
        // Check every property in a static json file whether match the response
    ...
    func test_with_missing_file_error_thrown() {
        // If file is an empty string, it should throw an error immediately
    ...
    func test_with_invalid_error_thrown() {
        // If file is an empty string, it should throw an error immediately
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "wrong_name", type: UsersResponse.self), "An error should be thrown")
        do {
            _ = try StaticJSONMapper.decode(file: "wrong_name", type: UsersResponse.self)
        } catch {
            guard let mappingError = error as? StaticJSONMapper.MappingError else {
                XCTFail("This is the wrong type of error for missing files")
                return
